// src/pages/Verify.jsx
import { useState, useEffect } from 'react'
import { Link, useNavigate, useLocation } from 'react-router-dom'
import { useLanguage } from '../context/LanguageContext'
import { verifyCode, resendCode } from '../api/auth'
import GridDistortion from '../components/GridDistortion';

const styles = `
  @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Mono:wght@300;400;500&display=swap');

  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

  .verify-split {
    display: flex;
    width: 100vw;
    height: 100vh;
    overflow: hidden;
    background: #060504;
    font-family: 'Syne', sans-serif;
  }

  /* ── LEFT PANEL ── */
  .verify-left {
    position: relative;
    width: 38%;
    min-width: 380px;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    padding: 48px 52px;
    background: rgba(12, 10, 9, 0.95);
    z-index: 2;
    overflow: hidden;
    border-right: 1px solid rgba(244, 237, 230, 0.05);
  }

  .verify-left::before {
    content: '';
    position: absolute;
    top: -80px;
    left: -80px;
    width: 320px;
    height: 320px;
    background: radial-gradient(circle, rgba(244, 237, 230, 0.08) 0%, transparent 70%);
    pointer-events: none;
  }

  /* ── TOP BAR ── */
  .verify-topbar {
    position: absolute;
    top: 0;
    left: 0;
    right: 0;
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 20px 52px;
    z-index: 10;
  }

  .verify-logo img {
    height: 70px;
    width: 70px;
    filter: brightness(1.2) grayscale(0.5);
  }

  .verify-logo-name {
    font-size: 15px;
    font-weight: 700;
    color: #F4EDE6;
    letter-spacing: 0.06em;
    text-transform: uppercase;
  }

  .verify-topbar-link {
    font-family: 'DM Mono', monospace;
    font-size: 11px;
    letter-spacing: 0.1em;
    color: rgba(244, 237, 230, 0.45);
    text-decoration: none;
    display: flex;
    align-items: center;
    gap: 6px;
    transition: color 0.2s;
  }

  .verify-topbar-link:hover {
    color: #F4EDE6;
  }

  /* ── CONTENT ── */
  .verify-content {
    position: relative;
    z-index: 1;
    display: flex;
    flex-direction: column;
    gap: 0;
  }

  .verify-eyebrow {
    font-family: 'DM Mono', monospace;
    font-size: 10px;
    letter-spacing: 0.22em;
    text-transform: uppercase;
    color: #F4EDE6;
    opacity: 0.7;
    margin-bottom: 12px;
    display: flex;
    align-items: center;
    gap: 8px;
  }

  .verify-eyebrow::before {
    content: '';
    display: block;
    width: 18px;
    height: 1px;
    background: rgba(244, 237, 230, 0.5);
  }

  .verify-heading {
    font-size: 30px;
    font-weight: 800;
    color: #F4EDE6;
    letter-spacing: -0.02em;
    line-height: 1.15;
    margin-bottom: 6px;
  }

  .verify-subheading {
    font-family: 'DM Mono', monospace;
    font-size: 11px;
    color: rgba(244, 237, 230, 0.32);
    letter-spacing: 0.04em;
    margin-bottom: 36px;
    line-height: 1.6;
  }

  .verify-field {
    display: flex;
    flex-direction: column;
    gap: 6px;
    margin-bottom: 14px;
  }

  .verify-field label {
    font-family: 'DM Mono', monospace;
    font-size: 10px;
    letter-spacing: 0.14em;
    text-transform: uppercase;
    color: rgba(244, 237, 230, 0.4);
  }

  .verify-field input {
    width: 100%;
    padding: 12px 16px;
    background: rgba(255, 255, 255, 0.02);
    border: 1px solid rgba(244, 237, 230, 0.1);
    border-radius: 9px;
    color: #F4EDE6;
    font-family: 'Syne', sans-serif;
    font-size: 18px;
    letter-spacing: 0.3em;
    text-align: center;
    outline: none;
    transition: all 0.2s;
  }

  .verify-field input:focus {
    border-color: rgba(244, 237, 230, 0.4);
    background: rgba(244, 237, 230, 0.04);
  }

  .verify-error {
    font-family: 'DM Mono', monospace;
    font-size: 11px;
    color: #E6B8A2;
    background: rgba(230, 184, 162, 0.08);
    border: 1px solid rgba(230, 184, 162, 0.15);
    border-radius: 8px;
    padding: 10px 14px;
    margin-bottom: 16px;
  }

  .verify-submit-btn {
    width: 100%;
    padding: 14px 20px;
    background: #F4EDE6;
    border: none;
    border-radius: 10px;
    color: #060504;
    font-family: 'Syne', sans-serif;
    font-size: 14px;
    font-weight: 700;
    letter-spacing: 0.02em;
    cursor: pointer;
    transition: opacity 0.2s, transform 0.15s;
    margin-top: 4px;
  }

  .verify-submit-btn:hover {
    opacity: 0.9;
    transform: translateY(-1px);
  }

  .verify-submit-btn:disabled {
    opacity: 0.5;
    transform: none;
    cursor: not-allowed;
  }

  .verify-resend-container {
    margin-top: 18px;
    text-align: center;
  }

  .verify-resend-btn {
    background: none;
    border: none;
    font-family: 'DM Mono', monospace;
    font-size: 11px;
    color: #F4EDE6;
    opacity: 0.4;
    cursor: pointer;
    transition: opacity 0.2s;
    text-decoration: underline;
  }

  .verify-resend-btn:hover:not(:disabled) {
    opacity: 0.8;
  }

  .verify-resend-btn:disabled {
    cursor: not-allowed;
  }

  .verify-terms {
    font-family: 'DM Mono', monospace;
    font-size: 9px;
    color: rgba(244, 237, 230, 0.15);
    letter-spacing: 0.08em;
    text-align: center;
    margin-top: 24px;
  }

  /* ── RIGHT PANEL ── */
  .verify-right {
    flex: 1;
    position: relative;
    overflow: hidden;
  }

  .verify-right-bg {
    position: absolute;
    inset: 0;
    background:
      radial-gradient(ellipse at 30% 20%, rgba(244, 237, 230, 0.08) 0%, transparent 55%),
      #0d0c0b;
  }

  .verify-character {
    position: absolute;
    bottom:0%;
    width: 100%;          
    height:auto;
    max-height:100%;        
    object-fit: contain; 
    mix-blend-mode: luminosity;
    opacity: 0.90;
    z-index: 2;
    pointer-events: none; 
  }

  .verify-right-fade {
    position: absolute;
    top: 0;
    left: 0;
    width: 120px;
    height: 100%;
    background: linear-gradient(90deg, #060504 0%, transparent 100%);
    pointer-events: none;
  }

  .verify-right-tag {
    position: absolute;
    top: 28px;
    right: 28px;
    font-family: 'DM Mono', monospace;
    font-size: 10px;
    letter-spacing: 0.18em;
    text-transform: uppercase;
    color: rgba(244, 237, 230, 0.35);
    display: flex;
    align-items: center;
    gap: 6px;
    z-index:2;
    cursor:pointer;
  }

  .verify-right-tag::after {
    content: '';
    display: block;
    width: 14px;
    height: 1px;
    background: rgba(244, 237, 230, 0.25);
  }

  .verify-right-bottom {
    position: absolute;
    bottom: 32px;
    left: 0;
    right: 0;
    display: flex;
    justify-content: center;
    pointer-events: none;
    z-index:2;
  }

  .verify-right-pill {
    font-family: 'DM Mono', monospace;
    font-size: 9px;
    letter-spacing: 0.2em;
    text-transform: uppercase;
    color: rgba(244, 237, 230, 0.35);
    background: rgba(244, 237, 230, 0.03);
    border: 1px solid rgba(244, 237, 230, 0.07);
    border-radius: 100px;
    padding: 6px 16px;
  }

  @media (max-width: 700px) {
    .verify-split { flex-direction: column; }
    .verify-left {
      width: 100%;
      min-width: unset;
      height: 100%;
      padding: 100px 28px 48px;
      border-right: none;
    }
    .verify-right { display: none; }
  }
`

export default function Verify() {
  const { t } = useLanguage()
  const navigate = useNavigate()
  const location = useLocation()
  
  const [code, setCode] = useState('')
  const [message, setMessage] = useState('')
  const [loading, setLoading] = useState(false)
  const [resendCooldown, setResendCooldown] = useState(0)
  const [resendStatus, setResendStatus] = useState('')

  const email = location.state?.email || ''

  useEffect(() => {
    if (!email) navigate('/login')
  }, [email, navigate])

  useEffect(() => {
    let timer
    if (resendCooldown > 0) {
      timer = setInterval(() => setResendCooldown(c => c - 1), 1000)
    }
    return () => clearInterval(timer)
  }, [resendCooldown])

  async function handleSubmit(e) {
    e.preventDefault()
    setMessage('')
    if (!code || code.length < 6) {
      setMessage(t('err_verify_fill'))
      return
    }
    setLoading(true)
    try {
      const data = await verifyCode({ email, code })
      localStorage.setItem('biar_token', data.token)
      localStorage.setItem('biar_user', JSON.stringify(data.user))
      navigate('/')
    } catch (err) {
      if (err.code === 'invalid_or_expired_code') setMessage(t('err_verify_invalid'))
      else setMessage(t('err_auth_server'))
    } finally {
      setLoading(false)
    }
  }

  async function handleResend() {
    if (resendCooldown > 0) return
    setResendStatus('')
    try {
      await resendCode({ email })
      setResendStatus(t('verify_sent'))
      setResendCooldown(60)
    } catch (err) {
      setMessage(t('err_auth_server'))
    }
  }

  return (
    <div className="verify-split">
      <style>{styles}</style>

      {/* Grid Distortion Background */}
      <div style={{ width: '100%', height: '100%', position:'absolute', zIndex:1, opacity:1 }}>
        <GridDistortion
          imageSrc="/images/wallpaper.png"
          grid={10}
          mouse={0.1}
          strength={0.15}
          relaxation={0.9}
        />
      </div>

      <div className="verify-left">
        <div className="verify-topbar">
          <Link to="/" className="verify-logo">
            <img src="/images/logo_biar2.png" alt="BIAR" />
          </Link>
          <Link to="/login" className="verify-topbar-link">
            {t('sign_signin')}
            <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
              <path fillRule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8"/>
            </svg>
          </Link>
        </div>

        <div className="verify-content">
          <p className="verify-eyebrow">{t('verify_title')}</p>
          <h1 className="verify-heading">{t('verify_title')}</h1>
          <p className="verify-subheading">
            {t('verify_sub')}<br/>
            <span style={{color:'#F4EDE6', opacity:0.8}}>{email}</span>
          </p>

          <form onSubmit={handleSubmit}>
            <div className="verify-field">
              <label>{t('verify_code_lbl')}</label>
              <input
                type="text"
                autoComplete="one-time-code"
                maxLength={6}
                placeholder="000 000"
                value={code}
                onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))}
              />
            </div>

            {message && <p className="verify-error">{message}</p>}

            <button type="submit" className="verify-submit-btn" disabled={loading}>
              {loading ? '...' : t('verify_btn')}
            </button>
          </form>

          <div className="verify-resend-container">
            <button 
              className="verify-resend-btn" 
              onClick={handleResend}
              disabled={resendCooldown > 0}
            >
              {resendCooldown > 0 
                ? t('verify_wait').replace('{s}', resendCooldown)
                : t('verify_resend')
              }
            </button>
            {resendStatus && (
              <p style={{fontFamily:'DM Mono', fontSize:'10px', color:'#F4EDE6', opacity:0.5, marginTop:'8px'}}>
                {resendStatus}
              </p>
            )}
          </div>

          <p className="verify-terms">{t('terms_policy')}</p>
        </div>
      </div>

      <div className="verify-right">
        <div className="verify-right-bg" />
        {/* Placeholder for character image to match complexity of Login/Register */}
        {/* <img src="/images/character.png" alt="" className="verify-character" /> */}
        <div className="verify-right-fade" />
        <Link to="/"><div className="verify-right-tag">HOME</div></Link>
        <div className="verify-right-bottom">
          <span className="verify-right-pill">{t('verify_title')} · BIAR</span>
        </div>
      </div>
    </div>
  )
}
